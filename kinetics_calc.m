...
foot_lenght = sqrt((LTOE_x - LANKLE_x).^2 + (LTOE_y - LANKLE_y).^2);
inertia_F = m_F * (0.475*foot_lenght).^2;
...
% Read left ankle angle data
ankleAngleL = angles_left.leftAnkleAngleL * pi/180; % angles in radians

l_alfa_a = [];
l_w_a = [];
for i=3:size(leftTimeRange_der, 2)
    w1 = (ankleAngleL(i-1) - ankleAngleL(i-2)) / timeStep;
    w2 = (ankleAngleL(i) - ankleAngleL(i-1)) / timeStep;
    l_w_a(i-2, 1) = w2;
    a = (w2 - w1) / timeStep;
    l_alfa_a(i-2, 1) = a;
end
...
l_alfa_f = [];
for i=3:size(leftTimeRange_der, 2)
    w1 = (footAngleL(i-1) - footAngleL(i-2)) / timeStep;
    w2 = (footAngleL(i) - footAngleL(i-1)) / timeStep;
    a = (w2 - w1) / timeStep;
    l_alfa_f(i-2, 1) = a;
end
...
l_moment_A = inertia_F(n+1:end) .* l_alfa_f - l_force_GR_x .* l_dist_GR_y(n+1:end) - l_force_GR_y .* l_dist_GR_x(n+1:end) + l_force_A_x .* l_dist_AF_y(n+1:end) + l_force_A_y .* l_dist_AF_x(n+1:end);

l_power_A = l_moment_A .* l_w_a;
...
foot_lenght = sqrt((RTOE_x - RANKLE_x).^2 + (RTOE_y - RANKLE_y).^2);
inertia_F = m_F * (0.475*foot_lenght).^2;
...
% Read ankle angle data
ankleAngleR = angles_right.rightAnkleAngleR * pi/180; % angles in radians

r_alfa_a = [];
r_w_a = [];
for i=3:size(rightTimeRange_der, 2)
    w1 = (ankleAngleR(i-1) - ankleAngleR(i-2)) / timeStep;
    w2 = (ankleAngleR(i) - ankleAngleR(i-1)) / timeStep;
    r_w_a(i-2, 1) = w2;
    a = (w2 - w1) / timeStep;
    r_alfa_a(i-2, 1) = a;
end
...
r_alfa_f = [];
for i=3:size(rightTimeRange_der, 2)
    w1 = (footAngleR(i-1) - footAngleR(i-2)) / timeStep;
    w2 = (footAngleR(i) - footAngleR(i-1)) / timeStep;
    a = (w2 - w1) / timeStep;
    r_alfa_f(i-2, 1) = a;
end
...
r_moment_A = inertia_F(n+1:end) .* r_alfa_f - r_force_GR_x .* r_dist_GR_y(n+1:end) - r_force_GR_y .* r_dist_GR_x(n+1:end)  + r_force_A_x .* r_dist_AF_y(n+1:end)  + r_force_A_y .* r_dist_AF_x(n+1:end) ;

r_power_A = r_moment_A .* r_w_a;
...
shank_length = sqrt((LKNEE_x - LANKLE_x).^2 + (LKNEE_y - LANKLE_y).^2);
inertia_S = m_S * (0.302*shank_length).^2;
...
% Read left knee angle data
kneeAngleL = angles_left.leftKneeAngleL* pi/180; % angles in radians

l_alfa_K = [];
l_w_K = [];
for i=3:size(leftTimeRange_der, 2)
    w1 = (kneeAngleL(i-1) - kneeAngleL(i-2)) / timeStep;
    w2 = (kneeAngleL(i) - kneeAngleL(i-1)) / timeStep;
    l_w_K(i-2, 1) = w2;
    a = (w2 - w1) / timeStep;
    l_alfa_K(i-2, 1) = a;
end
...
shnakAngleL = angles_left.leftShankAngleL* pi/180; % angles in radians

l_alfa_S = [];
for i=3:size(leftTimeRange_der, 2)
    w1 = (shnakAngleL(i-1) - shnakAngleL(i-2)) / timeStep;
    w2 = (shnakAngleL(i) - shnakAngleL(i-1)) / timeStep;
    a = (w2 - w1) / timeStep;
    l_alfa_S(i-2, 1) = a;
end
...
l_moment_K = inertia_S(n+1:end) .* l_alfa_S + l_moment_A + l_force_A_x .* l_dist_AS_y(n+1:end) - l_force_A_y .* l_dist_AS_x(n+1:end) - l_force_K_y .* l_dist_KS_x(n+1:end) + l_force_K_x .* l_dist_KS_y(n+1:end);

l_power_K = l_moment_K .* l_w_K;
...
shank_length = sqrt((RKNEE_x - RANKLE_x).^2 + (RKNEE_y - RANKLE_y).^2);
inertia_S = m_S * (0.302*shank_length).^2;
...
% Read left shank angle data
kneeAngleR = angles_right.rightKneeAngleR * pi/180; % angles in radians

r_alfa_K = [];
r_w_K = [];
for i=3:size(rightTimeRange_der, 2)
    w1 = (kneeAngleR(i-1) - kneeAngleR(i-2)) / timeStep;
    w2 = (kneeAngleR(i) - kneeAngleR(i-1)) / timeStep;
    r_w_K(i-2, 1) = w2;
    a = (w2 - w1) / timeStep;
    r_alfa_K(i-2, 1) = a;
end

% Read right shank angle data
shankAngleR = angles_right.rightShankAngleR* pi/180; % angles in radians

r_alfa_S = [];
for i=3:size(rightTimeRange_der, 2)
    w1 = (shankAngleR(i-1) - shankAngleR(i-2)) / timeStep;
    w2 = (shankAngleR(i) - shankAngleR(i-1)) / timeStep;
    a = (w2 - w1) / timeStep;
    r_alfa_S(i-2, 1) = a;
end
...
r_moment_K = inertia_S(n+1:end) .* r_alfa_S + r_moment_A + r_force_A_x .* r_dist_AS_y(n+1:end) - r_force_A_y .* r_dist_AS_x(n+1:end) - r_force_K_y .* r_dist_KS_x(n+1:end) + r_force_K_x .* r_dist_KS_y(n+1:end);

r_power_K = r_moment_K .* r_w_K;
...
thigh_length = sqrt((LHIP_x - LKNEE_x).^2 + (LHIP_y - LKNEE_y).^2);
inertia_T = m_T * (0.323*thigh_length).^2;
...
% Read left hip angle data
hipAngleL = angles_left.leftHipAngleL * pi/180; % angles in radians

l_alfa_H = [];
l_w_H = [];
for i=3:size(leftTimeRange_der, 2)
    w1 = (hipAngleL(i-1) - hipAngleL(i-2)) / timeStep;
    w2 = (hipAngleL(i) - hipAngleL(i-1)) / timeStep;
    l_w_H(i-2, 1) = w2;
    a = (w2 - w1) / timeStep;
    l_alfa_H(i-2, 1) = a;
end
...
l_moment_H = inertia_T(n+1:end) .* l_alfa_T + l_moment_K + l_force_K_x .* l_dist_kt_y(n+1:end) + l_force_K_y .* l_dist_kt_x(n+1:end) + l_force_H_y .* l_dist_ht_x(n+1:end) + l_force_H_x .* l_dist_ht_y(n+1:end);

l_power_H = l_moment_H .* l_w_H;
...
thigh_length = sqrt((RHIP_x - RKNEE_x).^2 + (RHIP_y - RKNEE_y).^2);
inertia_T = m_T * (0.323*thigh_length).^2;
...
hipAngleR = angles_right.rightHipAngleR * pi/180; % angles in radians

r_alfa_H = [];
r_w_H = [];
for i=3:size(rightTimeRange_der, 2)
    w1 = (hipAngleR(i-1) - hipAngleR(i-2)) / timeStep;
    w2 = (hipAngleR(i) - hipAngleR(i-1)) / timeStep;
    r_w_H(i-2, 1) = w2;
    a = (w2 - w1) / timeStep;
    r_alfa_H(i-2, 1) = a;
end
...
r_alfa_T = [];
for i=3:size(rightTimeRange_der, 2)
    w1 = (thighAngleR(i-1) - thighAngleR(i-2)) / timeStep;
    w2 = (thighAngleR(i) - thighAngleR(i-1)) / timeStep;
    a = (w2 - w1) / timeStep;
    r_alfa_T(i-2, 1) = a;
end
...
r_moment_H = inertia_T(n+1:end) .* -r_alfa_T + r_moment_K + r_force_K_x .* r_dist_kt_y(n+1:end) + r_force_K_y .* r_dist_kt_x(n+1:end) + r_force_H_y .* r_dist_ht_x(n+1:end) + r_force_H_x .* r_dist_ht_y(n+1:end);

r_power_H = r_moment_H .* r_w_H;
...
title('Thigh angular acceleration')
legend('Right gait', 'Left gait')
xlabel('Gait cycle [%]')
ylabel('Angular acceleration [rad/s^2]', 'FontSize', 9)
%axis([0 100 -40 30])
...
%% SAVE DATA
if index == 1
    walking_table = table(r_alfa_a, r_moment_A, r_power_A, r_alfa_K, r_moment_K, r_power_K, r_alfa_H, r_moment_H, r_power_H);
    writetable(walking_table,'walking_kinetics.txt', 'Delimiter',' ')
elseif index == 2
    jogging_table = table(r_alfa_a, r_moment_A, r_power_A, r_alfa_K, r_moment_K, r_power_K, r_alfa_H, r_moment_H, r_power_H);
